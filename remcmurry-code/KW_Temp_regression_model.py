...
tl_size=15      #sets font size for graphs
outputGraphs=True  # do you want plots saved?
createCSV=True     # do you want csv files of dataframes?
...
ax.set_position(pos=[.065,.10,.9,.85])
plt.plot(WFopen[:,0],WFopen[:,1],'.',c='darkorange',label='Open Hours Baseline',
             alpha=alFA)
plt.plot(evWFopen[:,0],evWFopen[:,1],'.',c='darkgreen',label='Open Hours post Retrofit',
             alpha=alFA)
...
plt.plot(WFclose[:,0],WFclose[:,1],'.',c='darkorange',label='Closed Hours Baseline',
             alpha=alFA)
plt.plot(evWFclose[:,0],evWFclose[:,1],'.',c='darkgreen',label='Closed Hours Post Retrofit',
             alpha=alFA)
#plt.ylabel('kW for WF Site')
#plt.xlabel('Outdoor Air Temp ($^\circ$F)')
#ax2.set_xlim(20,110)
...
plotname='Baseline and Post Retrofit data clouds for closed and open hours'
...
kWfit=res.params[0]+res.params[1]*(Tfit)+res.params[2]*(np.maximum(Tfit-BPbest,0))
...
plt.plot(pgeDFev.index[tstar:tend].to_pydatetime(),pgeDFev.pgkW_Site_pred[tstar:tend],'r-',c='darkorange')
plt.plot(pgeDFev.index[tstar:tend].to_pydatetime(),pgeDFev.pgkW_Site[tstar:tend],'g-',c='darkgreen')
ax.xaxis.set_major_formatter(hfmt)
ax.set_ylim(0,350)
...
if outputGraphs: plt.savefig(outputPath+str(st_YrEv)+'_'+str(placeHolder)+\
                             str(obsmonth)+'_'+str(dstart+1)+' '+str(dnum)+plotname+'.png',
                             bbox_to_inches='tight')

#%%  Write out CSV files of used dataframes
if createCSV: 
    pgeDFev.to_csv(outputPath+str(st_YrEv)+'_'+str(placeHolder)+\
                        str(obsmonth)+'_pgeDFev.csv',na_rep='=NA()')
    pgeDFtr.to_csv(outputPath+str(st_YrEv)+'_'+str(placeHolder)+\
                        str(obsmonth)+'_pgeDFtr.csv',na_rep='=NA()')
    pgehourDF.to_csv(outputPath+str(st_YrEv)+'_'+str(placeHolder)+\
                        str(obsmonth)+'_pgehourDF.csv',na_rep='=NA()')
    pgedf.to_csv(outputPath+str(st_YrEv)+'_'+str(placeHolder)+\
                        str(obsmonth)+'_pgedf.csv',na_rep='=NA()')
    WFopenDF.to_csv(outputPath+str(st_YrEv)+'_'+str(placeHolder)+\
                        str(obsmonth)+'_WFopenDF.csv',na_rep='=NA()')
    WFcloseDF.to_csv(outputPath+str(st_YrEv)+'_'+str(placeHolder)+\
                        str(obsmonth)+'_WFcloseDF.csv',na_rep='=NA()')
    WFtransDF.to_csv(outputPath+str(st_YrEv)+'_'+str(placeHolder)+\
                        str(obsmonth)+'_WFtransDF.csv',na_rep='=NA()')
    evWFopenDF.to_csv(outputPath+str(st_YrEv)+'_'+str(placeHolder)+\
                        str(obsmonth)+'_evWFopenDF.csv',na_rep='=NA()')
    evWFcloseDF.to_csv(outputPath+str(st_YrEv)+'_'+str(placeHolder)+\
                        str(obsmonth)+'_evWFcloseDF.csv',na_rep='=NA()')
    evWFtransDF.to_csv(outputPath+str(st_YrEv)+'_'+str(placeHolder)+\
                        str(obsmonth)+'_evWFtransDF.csv',na_rep='=NA()')
...
ARendog=np.array([pgeDFtr.pgkW_Site[int(starttr*1440):int(endtr*1440)]])
ARexog=np.array([pgeDFtr.pgOA_Temp[int(starttr*1440):int(endtr*1440)]])
ARindex=np.array([pgeDFtr.index[int(starttr*1440):int(endtr*1440)]])

model = sm.tsa.ARMA(ARendog[1:],(10,1))
