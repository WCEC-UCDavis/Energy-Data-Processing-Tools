...
#pardf['opMode']=['Transient']*len(df)
#pardf['opModeStatus']=['Transient']*len(df)
...
Panel_Status=[0]*len(df)
RackA_Status=[0]*len(df)
RackB_Status=[0]*len(df)

for x,iAC36,iAC30,iAC31,iPanel,iRackA,iRackB in zip(
        range(len(pardf)),pardf.kW_AC36.values,pardf.kW_AC30.values,
        pardf.kW_AC31.values,pardf.kW_Panel.values,pardf.kW_RackA.values,
        pardf.kW_RackB.values):
    ...
    if iRackA > RackAThreshLo:
        if iRackA > RackAThreshHi:
            RackA_Status[x]=2
        else:
            RackA_Status[x]=1
        
    if iRackB > RackBThreshLo:
        if iRackB > RackBThreshHi:
            RackB_Status[x]=2
        else:
            RackB_Status[x]=1
...
pardf['RackA_Status']=RackA_Status
pardf['RackB_Status']=RackB_Status


#defining component boolean states
AC36VENT=(pardf.AC36_Status==1) | (pardf.kW_AC36>=AC36Thresh)
AC30VENT=(pardf.AC30_Status==1) | (pardf.kW_AC30>=AC30Thresh)
AC31VENT=(pardf.AC31_Status==1) | (pardf.kW_AC31>=AC31Thresh)
RackALO=(pardf.RackA_Status==0) | (pardf.kW_RackA<RackAThreshLo)
RackAMD=(pardf.RackA_Status==1) & (pardf.kW_RackA>=RackAThreshLo)
RackAHI=(pardf.RackA_Status==2) & (pardf.kW_RackA>=RackAThreshHi)
RackBLO=(pardf.RackB_Status==0) | (pardf.kW_RackB<RackBThreshLo)
RackBMD=(pardf.RackB_Status==1) & (pardf.kW_RackB>=RackBThreshLo)
RackBHI=(pardf.RackB_Status==2) & (pardf.kW_RackB>=RackBThreshHi)
...
for mode,modeName in zip(allModes,allModeNames):
    for i in df[mode].index:
        df.opMode[i]=modeName
...
for x,iC1,iC2,iF1,iF2,iPump,iOA,iSA,iPow in zip(
...
for calc in ['vDotSA','vDotOA','wOA','wSA','wRA','OAwb','OAwbd','mCPmin',
...
for calc in ['vDotSA','vDotOA','wOA','wSA','wRA','OAwb','OAwbd',
...
for mode in modes:
    plotmodes[modes.index(mode)]=mode
...
for mode in modes:
    if mode=='VENT':
        ...
        md_colors[modes.index(mode)]=mode_colors[mode_type.index(mode)]
        plotmodes[mode_type.index(mode+'_heat')]='senSystem_'+mode+'_heat'
        md_colors[mode_type.index(mode+'_heat')]=mode_colors[mode_type.index(mode+'_heat')]
    else:
...
for mode in modes:
    if mode=='VENT':
        ...
        md_colors[modes.index(mode)]=mode_colors[mode_type.index(mode)]
        plotmodes[mode_type.index(mode+'_heat')]='latSystem_'+mode+'_heat'
        md_colors[mode_type.index(mode+'_heat')]=mode_colors[mode_type.index(mode+'_heat')]
    else:
...
for mode in modes:
    if mode=='VENT':
        ...
        md_colors[modes.index(mode)]=mode_colors[mode_type.index(mode)]
        plotmodes[mode_type.index(mode+'_heat')]='senRoom_'+mode+'_heat'
        md_colors[mode_type.index(mode+'_heat')]=mode_colors[mode_type.index(mode+'_heat')]
    else:
...
modes=['VENT', 'IEC ONLY', 'IEC and DX1', 'IEC and DX2', 'IEC and HEAT1',
           'HEAT1', 'OFF']
md_colors=['#000000' for mc in range(len(modes))]           
plotmodes=[None for pm in range(len(modes))]           
for mode in modes:
...
for key,data in df.groupby(['opMode','opModeStatus']):
    if 'Transient' not in key and 'OFF' not in key:
        ...
        plt.plot(data.OA_Temp,data.senSystemCap,'.',c=mColor,label=key[0],
                 alpha=alFA)
for item in ([ax.title, ax1.xaxis.label, ax.yaxis.label] +
             ax.get_xticklabels() + ax.get_yticklabels()):
    item.set_fontsize(tl_size)
#plt.legend(bbox_to_anchor=(0. ,1.02 ,1.,0.3),loc=8,ncol=5,mode='expand',
#           borderaxespad=0,prop={'size':9},numpoints=1)
...
#%% OA Temp vs SA Temp with off and transients
fig=plt.figure()
fig.set_size_inches(wFig,hFig)
ax=fig.add_subplot(111)
for key,data in df.groupby(['opMode','opModeStatus']):
...
plotname='OA Temp vs SA Temp inc. off and trasient'
...
#%% RA Temp vs SA Temp with off and transients
